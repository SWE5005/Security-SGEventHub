...
//    @PreAuthorize("hasAuthority('SCOPE_USER')")
    ...
    public ResponseEntity<?> getAllEventUsers(Principal principal) {
        ...
        try {
            ...
            ApiResponse<String> response =
                    new ApiResponse<>("Failed to get user list", HttpStatus.INTERNAL_SERVER_ERROR.value(), null);
    ...
    public ResponseEntity<?> createUser(@RequestBody CreateUserRequest userRequest) {
        log.info("[UserController:createUser]Request to create user started for user: ",userRequest.getEmailAddress());
        try {
            ...
            ApiResponse<String> response =
                    new ApiResponse<>("Failed to create user", HttpStatus.INTERNAL_SERVER_ERROR.value(), null);
            return new ResponseEntity<>(response, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
...
//    @PreAuthorize("hasAuthority('SCOPE_USER')")
    ...
    public ResponseEntity<?> getUserDetails(Principal principal, @PathVariable @Valid @NotNull String userId) {
        ...
        try {
            ...
            log.error("[UserController:getUserDetails] Failed to get user details", e);
            ApiResponse<String> response =
                    new ApiResponse<>("Failed to get user details", HttpStatus.INTERNAL_SERVER_ERROR.value(), null);
            return new ResponseEntity<>(response, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    @GetMapping (path="/{email}/details")
    public ResponseEntity<?> getUserDetailsByEmail(@PathVariable @Valid @NotNull String email) {
        log.info("[UserController:getUserDetails]Request to get user details started for user: ",email);
        try {
            return ResponseEntity.ok(manageUserService.getUserDetails(email));
        }catch (Exception e) {
            log.error("[UserController:getUserDetails] Failed to get user details", e);
            ApiResponse<String> response =
                    new ApiResponse<>("Failed to get user details", HttpStatus.INTERNAL_SERVER_ERROR.value(), null);
...
//    @PreAuthorize("hasAuthority('SCOPE_USER')")
    ...
    public ResponseEntity<?> updateUser(Principal principal,@RequestBody ManageUserRequest userRequest) {
        log.info("[UserController:updateUser]Request to update user started for user: ",principal.getName());
        try {
            ...
            ApiResponse<String> response =
                    new ApiResponse<>("Failed to update user", HttpStatus.INTERNAL_SERVER_ERROR.value(), null);
            return new ResponseEntity<>(response, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<?> deleteUser(@PathVariable @Valid @NotNull String userId) {
        log.info("[UserController:deleteUser]Request to delete user started for user: ");
        try {
            ...
            ApiResponse<String> response =
                    new ApiResponse<>("Failed to delete user", HttpStatus.INTERNAL_SERVER_ERROR.value(), null);
            return new ResponseEntity<>(response, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
